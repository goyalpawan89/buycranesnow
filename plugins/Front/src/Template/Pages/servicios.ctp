				...
				<tr>
					...
					<td>
						...
						<article class="category-description description page_description">
							...
							<?php if(!empty($content->archive_id)) { echo '<aside class="img_aside_page">'.$this->Html->image($this->Image->url($content->archive_id, 'full'), ['class' => 'img_page', 'alt' => $content->name]).'</aside>'; } ?>

								<aside class="content_services"><?= $content->description; ?></aside>
