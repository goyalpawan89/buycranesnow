...
<?php foreach ($posts as $key => $post)   {
        ...
        $price = $this->Number->currency($precio, $info['currency'], ['after']);
        ...
        $state = $this->Get->get_field_by_post_id($post['id'], 'state');
        ...
        $avalibleName = 'avalible_'.$avalible;
                            ...
                            if($avalible == 11) { $btnAvalible = 'rent'; } else {  $btnAvalible = 'sell'; } // boton comprar  o alquilar
                            if($authUser) { $btnAction = $this->Get->get_url_translate_popup($post->id); $classAvalible = 'offer_iframe'; $iframe = 'iframe'; } else {  $btnAction = '#login'; $classAvalible = 'fancybox'; $iframe = ''; }
                            if($video && !empty($video)) { $classVideo = 'list-post_image_video'; } else { $classVideo = ''; } ?>


                          <article id="<?= $this->request->params['action']; ?>" class="element-item list-post border1 <?= $disponible ?>-item" 
                                                  data-brand="<?= $marca; ?>" 
                                                  data-id="<?= $post['id']; ?>" 
                                                  data-year="<?= $year; ?>" 
                                                  data-tons="<?= $tons; ?>" 
                            ...
                            <aside class="list-post_text" data-price="<?= $precio; ?>">
                              <div>
                                <h2 class="list-post_title">
                                    <font class="list_post_title_font"><?= $this->Html->link(__($post['name']), $link, ['class' => 'color3']); ?> |</font> <?= __($extras['code']); ?>-<?= $post['id']; ?>
                                </h2> 
                                <?php if(!empty($precio)) { ?><span class="list-post_prince"><?php if($avalible == 11) { echo $extras['price_month']; ?>: <?php } ?> <?= $price; ?></span><?php } ?>
                              </div>

                              <?php $datos = ['maker' => $marca, 'year' => $year, 'model' => $model, 'tons' => $tons.' '.$extras['tons'], 'state' => $country." - ".$city]; 
                            ...
                            ?>
                                ...
                                <aside class="item_only-post_aside table-cell">
                                        <?php if(!empty($precio)) { echo $this->Html->link($price, $link, ['class' => 'color3 colorh4']); } ?>
                                </aside>
                              ...
                              <aside class="list-post_data">
                                ...
                                <span class="list-post_avalible"><?php if(isset($avalibleName)) { echo __($extras[$avalibleName]); } ?></span>
                                <div class="list-post_data_map">
                                  <a href="<?= $this->Get->get_url_translate_map($post->id); ?>" class="fancy_iframe" data-fancybox-type="iframe" data-fancybox-width="400" title="<?= __($extras['state']); ?> <?= $post->name; ?>"><i class="fa fa-map-marker color1 colorh3"></i></a>
                                </div>
                                <div class="list-post_data_rent">
                                  <?php if(!empty($tons)) { ?><p><strong><?= __($extras['tons']); ?>: </strong> <font><?= $tons; ?></font></p><?php } ?>
                                  <?php if(!empty($city)) { ?><p><font><?= $city; ?></font> - <font><?= $country; ?></font></p><?php } ?>
                                <div>
                              </aside>
...
<?= $this->Html->script('Front.front/isotopo'); ?>
