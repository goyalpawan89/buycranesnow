				...
				<tr class="fondo2 color5">
						...
						if($this->request->params['controller'] == 'Posts') { $tabName = 'categories'; $tabValue = $post->categories; } else { $tabName = ''; $tabValue = ''; } ?>
								...
								<tr>
									...
									<?php $datos = ['id' => $post->id,
													'name' => $post->name, $tabName => $tabValue, 
													'author' => $post->user->company_name, 
													'maker' => $this->Get->get_field_by_post_id($post->id, 'brand'),
													'avalible' => __d('administrator', $this->Get->get_field_by_post_id($post->id, 'avalible')),
													'capacity' => $this->Get->get_field_by_post_id($post->id, 'tons'),
